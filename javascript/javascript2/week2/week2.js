...
const numOfMoviesFrom80s = movies.filter(
  (movie) => movie.year >= 1980 && movie.year <= 1989
).length;
console.log(numOfMoviesFrom80s);
//Task-4
...
const moviesRatingAbove6 = movies
  .filter((movie) => movie.rating > 6)
  .map((movie) => movie.rating);
console.log(moviesRatingAbove6);

//Task-6
//Count the total number of movies containing any of following keywords:
//Surfer, Alien or Benjamin. So if there were 3 movies that contained Surfer, 1 with Alien and 2 with Benjamin, you would return 6. Can you make sure the search is case insensitive?

const moviesTitleWithKeywords = movies.filter(
  (movie) =>
...
const moviesTitleDuplicated = movies.filter((movie) => {
  let title = movie.title.toLowerCase().split(" ");
  newTitle = title.map((a) => removeSpecials(a));
  const duplicatedTitleMovies = newTitle.some(
    (word) => newTitle.indexOf(word) !== newTitle.lastIndexOf(word)
  );
  return duplicatedTitleMovies;
});

//Task-8
//Calculate the average rating of all the movies using reduce. Optional
let averageRating =
  movies.map((movie) => movie.rating).reduce((a, b) => a + b) /
...
function totalMoviesWithTags() {
  let goodMovies = moviesWithTag.filter((movie) => movie.tag == "Good").length;
  let averageMovies = moviesWithTag.filter(
    (movie) => movie.tag == "Average"
  ).length;
  let badMovies = moviesWithTag.filter((movie) => movie.tag == "Bad").length;
